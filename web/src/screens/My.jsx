...
export function My() {
  ...
  // 실제 store 데이터 기반 통계
  const stats = [
    { label: { ko: '방문', en: 'Visited', ja: '訪問', zh: '访问', es: 'Visitas' }, value: Object.keys(visited).length },
    { label: { ko: '저장', en: 'Saved', ja: '保存', zh: '收藏', es: 'Guardados' }, value: Object.keys(saved).length },
    { label: { ko: '퀴즈', en: 'Quiz', ja: 'クイズ', zh: '测验', es: 'Quiz' }, value: quizCount },
  ]
  const settings = [
    { label: { ko: '언어 설정', en: 'Language', ja: '言語設定', zh: '语言设置', es: 'Idioma' }, value: langLabel },
    { label: { ko: '알림 설정', en: 'Notifications', ja: '通知設定', zh: '通知设置', es: 'Notificaciones' }, onClick: () => nav('/notifications') },
    { label: { ko: '저장한 문화유산', en: 'Saved heritage', ja: '保存した文化遺産', zh: '收藏的文化遗产', es: 'Patrimonio guardado' }, value: Object.keys(saved).length, onClick: () => nav('/saved') },
    { label: { ko: '둘러보기', en: 'Explore map', ja: '地図で探す', zh: '浏览地图', es: 'Explorar mapa' }, onClick: () => nav('/map') },
  ]
  ...
  return (
    <div className="pt-16 px-content">
      ...
      <div className="flex items-center gap-3 mt-5">
        ...
        <div className="flex-1"><div className="font-bold text-[16px]">{t.myExplorer}</div></div>
        <LangToggle />
      </div>
      <div className="flex gap-3 mt-5">
        {stats.map((st, i) => (
          <div key={i} className="flex-1 rounded-card bg-black/5 py-4 text-center">
            ...
            <div className="text-[12px] text-black/55">{tr(st.label, lang)}</div>
          </div>
        ))}
      </div>
